...
export function ToolListItem({
  ...
  return (
    <div className="group bg-slate-900/60 hover:bg-slate-800/80 rounded-xl px-4 py-2.5 flex items-center justify-between gap-4 transition-all duration-150 border-0">
      {/* 左側: ツール名 ＆ 説明 */}
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          onClick={handleOpen}
          className="flex items-center gap-1 px-3 py-1 bg-slate-800/90 hover:bg-blue-600 text-slate-200 hover:text-white text-xs font-medium rounded-lg transition-all border border-slate-700/40 group/btn"
        >
          開く
        ...
        {canEdit && (
          <button
            onClick={() => onEdit(tool)}
            className="flex items-center gap-1 px-2.5 py-1 bg-slate-900/90 hover:bg-slate-800 text-slate-300 hover:text-blue-400 text-xs rounded-lg border border-slate-800 transition-colors"
            title="ツールの内容を編集"
          >
